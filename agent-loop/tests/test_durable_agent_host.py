...
class DurableAgentHostTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_recoverable_model_runtime_继续使用正式_stream_fn(self) -> None:
        provider = ScriptedProvider([
            assistant_message(
                model=self.model,
                content=[{"type": "text", "text": "恢复模型成功"}],
            )
        ])
        ...
        self.assertEqual(result["content"][0]["text"], "恢复模型成功")
        self.assertEqual(provider.call_count, 1)
        self.assertEqual(provider.contexts[0]["tools"][0]["name"], "divide")

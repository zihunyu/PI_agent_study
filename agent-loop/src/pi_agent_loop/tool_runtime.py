...
class ToolScheduler:
    ...
    async def acquire(
        ...
        *,
        ...
        declared_tenant = (
            tool.resolve_tenant_id(args) if tool.resolve_tenant_id else None
        )
        ...
        resource_keys = _resource_keys(tool, args)
...
class ToolDispatchRuntime:
    ...
    async def prepare(
        ...
        *,
        ...
        if tool is None:
            return ImmediateToolCall(error_tool_result(f"工具不存在：{tool_name}"), True)
        try:
            ...
            if self.authorize is not None:
                allowed = await _maybe_await(self.authorize(tool, args, identity, approval))
                if not allowed:
            ...
            if cancellation.cancelled:
                return _cancelled_before_dispatch(tool_name)
            return PreparedToolCall(tool_call, tool, args)
        except Exception as error:
            return ImmediateToolCall(error_tool_result(str(error)), True)
    ...
    async def dispatch(
        ...
        *,
        ...
        await _emit_tool_start(tool_call, sink)
        ...
        if isinstance(prepared, ImmediateToolCall):
            outcome = ToolDispatchOutcome(tool_call, prepared.result, prepared.is_error)
            ...
            prepared.trusted_tenant_id = self._trusted_tenant_id(tenant_id)
    ...
    async def dispatch_many(
        ...
        *,
        ...
        trusted_tenant_id = self._trusted_tenant_id(tenant_id)
            ...
            if isinstance(prepared, ImmediateToolCall):
                ...
                await _emit(
                    emit,
                    {
                        ...
                        "resourceKeyCount": len(_resource_keys(prepared.tool, prepared.args)),
                        "priority": prepared.tool.priority,
                    },
                )
        ...
        if execution == "sequential":
            await self._run_sequential(
                scheduled, slots, context, assistant_message, cancellation, emit
            )
        else:
            ...
            for indexed in scheduled:
                if _execution_mode(indexed[1].tool, execution) == "exclusive":
                    await self._run_group(
                        pending, slots, context, assistant_message, cancellation, emit
                    )
                    pending = []
                    await self._run_group(
                        [indexed], slots, context, assistant_message, cancellation, emit
                    )
                else:
        ...
        try:
            ...
            if execution in done:
                try:
                    ...
                    return error_tool_result(
                        str(error), details={"code": "resource_lock_timeout"}
                    ), True
                    ...
                    return error_tool_result(
                        str(error), details={"code": "tool_execution_error"}
                    ), True
